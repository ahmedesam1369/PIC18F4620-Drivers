...
Std_ReturnType Timer0_Init(const timer0_t *_timer){
    ...
    if(NULL == _timer){
       ...
       if(INTERRUPT_HIGH_PRIORITY == _timer->priority){
           ...
           TIMER0_InterruptHighPriority();
       }else if(INTERRUPT_LOW_PRIORITY == _timer->priority){
           INTERRUPT_GlobalInterruptLowEnable();
           TIMER0_InterruptLowPriority();
       }else{/* Nothing */}
       #else
...
static inline void Timer0_Prescaler_Config(const timer0_t *_timer){
   if(TIMER0_PRESCALLER_ENABLE_CFG == _timer->prescaler_enable){
           TIMER0_PRESCALLER_ENABLE();
           T0CONbits.T0PS = _timer->prescaler_value;
       }else if(TIMER0_PRESCALLER_DISABLE_CFG == _timer->prescaler_enable){
...
static inline void Timer0_Mode_Select(const timer0_t *_timer){
   if(TIMER0_TIMER_MODE == _timer->timer0_mode){
            ...
            }else if(TIMER0_COUNTER_FAILING_EDGE_CFG == _timer->timer0_counter_edge){
